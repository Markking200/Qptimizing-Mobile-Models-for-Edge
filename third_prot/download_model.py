...
def static_download(path: str) -> None:
...
def cmd_infer(ctx: Context) -> None:
    ...
    #if model_name not in mod_reg:
    #    raise ValueError(f"Modell '{model_name}' aus metadata.json unbekannt. Unterstützt: {', '.join(mod_reg)}")
    ...
    #logger.info(f"{sd_path}")
    #logger.info(ts_path.exists())

    if ts_path.exists():
        logger.info(f"Lade TorchScript: {ts_path}")
        model = torch.jit.load(str(ts_path), map_location=device)
        print("Checkpoint 1")
        model.eval()
    else:
        logger.error(f"Could not find {model_name} ind {ts_path}")
    #     logging.info(f"Lade state_dict + Architektur für {model_name}")
    #     ctor, _ = mod_reg[model_name]
    ...
    # Bild laden
    imp = "../data/apple.jpg"
    img_path = Path(imp).expanduser().resolve()
    ...
    topk = 5#int(args.topk)
    idxs = np.argsort(probs)[::-1][:topk]
    logger.info(f"Top-{topk} Ergebnisse:")
